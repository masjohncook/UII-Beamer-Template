...
\includegraphics[width=\textwidth]{slides/main_matter/assets/min_fuel.png}
...
\end{column}
\begin{column}{0.35\textwidth}
\includegraphics[width=\textwidth]{slides/main_matter/assets/min_time.png}
...
\begin{sidepic}{images/placeholder.png}{Side-Picture Slides}
...
\begin{chapter}[images/Beranda-2-1.png]{uiiblue}{Personalization}
...
\begin{itemize}
    \item We are currently using \testcolor{uiiblue} color for our footer.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
For more options with fonts, you need to modify the self explanatory \verb|uiifont.sty| file. Next few slides will be helpful in this regard.
\end{frame}
...
\end{itemize}
UII's primary font is Open Sans, as specified in the UII brand guidelines.
\begin{itemize}
\item Open Sans is included in the \verb|fonts| folder and loaded automatically.
\item For additional font options, refer to the \hrefcol{https://tug.org/FontCatalogue/}{LaTeX Font Catalogue}.
\end{itemize}
\end{frame}
...
\item The colors are defined in the \texttt{uiicolor} package, in accordance with the UII brand guidelines:
  \begin{itemize}
  \item UII's primary colors are UII Blue (\testcolor{uiiblue}) and UII Red (\testcolor{uiired}).
  \item UII's secondary colors are Chicago Blue (\testcolor{chicagoblue}), UI Health Teal (\testcolor{uihteal}) and Champions Gold (\testcolor{championsgold}).
  \item UII's neutral colors are Expo White (\testcolor{expowhite}) and Steel Gray (\testcolor{steelgray} or \testcolor{steelgrey}).
  \end{itemize}
...
\item Multiple background images have been provided in this template which may be used for chapter slides:
  \begin{itemize}
  \item Some UII campus photos and;
  \item the UII lockup logo.
...
\begin{chapter}[images/Beranda-2-1.png]{uiiblue}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}

\themecolor{lightnologo}
% These backgrounds already have the UII logo, so we temporarily change the theme to lightnologo
\begin{chapter}[images/uii_lockup.png]{uiiblue}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}

\begin{chapter}[images/placeholder.png]{uiiblue}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}

\begin{chapter}[images/placeholder.png]{steelgray}{Title goes here}
\textit{Subtitle goes here}
\end{chapter}
\themecolor{light}

\begin{frame}[fragile]{Other backgrounds}
\begin{itemize}
\item You can use the generic \verb|uii_lockup.png| background, or
\item Replace the images in the \verb|images| folder with your own.
    \begin{itemize}
    \item Supported formats: \verb|.png|, \verb|.jpg|, \verb|.pdf|.
    \end{itemize}
\item Download official UII visual assets from the UII brand guidelines portal.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item This template is based on the UIC Beamer Template by Usama Muneeb
    \begin{itemize}
    \item  \verb|https://github.com/usamamuneeb/uic-beamer-template|
    \end{itemize}
\item Adapted for Universitas Islam Indonesia (UII).
\item For questions or suggestions, contact your template maintainer.
\end{itemize}
\end{frame}